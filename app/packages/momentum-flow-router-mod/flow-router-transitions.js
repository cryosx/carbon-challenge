...
function CreateTransitions(Transitions, LastPage, Page, forwardTransition, backTransition) {
  // Create forward and reverse transition based on args
  // console.log('From: ' + LastPage + ' to ' + Page);
  var ForwardPageKey = LastPage + '->' + Page;
  Transitions[ForwardPageKey] = forwardTransition;
  var ReversePageKey = Page + '->' + LastPage;
  Transitions[ReversePageKey] = backTransition;
  return Transitions;
};
...
Transitioner = {
    ...
    TransitionOrder: function (Pages, forwardTransition, backTransition) {
      // Init Vars
      var Transitions = {};
      var LastPage = '';
      ...
      while (PageIndex < Pages.length) {
        ...
        while (LastIndex < PageIndex) {
          Transitions = CreateTransitions(Transitions, Pages[LastIndex], Pages[PageIndex], forwardTransition, backTransition);
          LastIndex++;
        }
        PageIndex++;
      }
